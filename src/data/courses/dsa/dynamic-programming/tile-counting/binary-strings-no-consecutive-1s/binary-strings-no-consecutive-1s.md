...
> **Key insight:** dp[i] = dp[i-1] + dp[i-2]. If position i is 0: any valid string of length i-1. If 1: position i-1 must be 0, so valid strings of length i-2.
...
> **Key Insight for Improvement:** Memoize computed states (top-down) or build bottom-up (tabulation). Recurrence: dp[i] = dp[i-1] + dp[i-2] (with dp[1]=2, dp[2]=3)

<br>

---

## Solution 2: DP — Fibonacci variant

**Recurrence:** `dp[i] = dp[i-1] + dp[i-2] (with dp[1]=2, dp[2]=3)`

**Algorithm:** Fibonacci variant
...
**Example walkthrough:**
```
dp[1]=2, dp[2]=3, dp[3]=5, dp[4]=8, dp[5]=13
```
