...
> **Key Insight for Improvement:** Memoize or tabulate. Recurrence: `dp[i][j]: if p[j]=='*': dp[i][j-2] (zero) or (match and dp[i-1][j]) (one+); if '.' or match: dp[i-1][j-1]`
...
**Recurrence:** `dp[i][j]: if p[j]=='*': dp[i][j-2] (zero) or (match and dp[i-1][j]) (one+); if '.' or match: dp[i-1][j-1]`
...
**Example walkthrough:**
```
Apply recurrence: dp[i][j]: if p[j]=='*': dp[i][j-2] (zero) or (match and dp[i-1][j]) (one+); if '.' or match: dp[i-1][j-1]
```
